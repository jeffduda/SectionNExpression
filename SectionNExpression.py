...
def main():
    ...
    eng.set_overwrite(args.force)
    eng.set_atlas_path(args.atlas_path)
    eng.set_path(args.path)
    
    specimenList = eng.get_specimens()
    print( eng.specimenDatasets )

    if not args.specimen is None:
        specimenList = [args.specimen]
    ...
    for count, s in enumerate(specimenList):
        ...
        if specimen_exists:
            ...
            oImgName = os.path.join(args.outdir, str(s)+"_"+age+"_"+dir+"_section.nii.gz")
            oExpName = os.path.join(args.outdir, str(s)+"_"+age+"_"+dir+"_expression.nii.gz")

            if not os.path.exists(oImgName) or args.force:
                ...
                volumeInfo.to_csv(os.path.join(args.outdir, str(s)+"_"+age+"_"+dir+"_metainfo.csv"), index=False)
        ...
        #if not os.path.exists(oExpName) or args.force:
